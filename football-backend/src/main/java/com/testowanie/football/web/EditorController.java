...
public class EditorController {
    ...
    @GetMapping
    public ResponseEntity<Page<EditorResource>> getEditors(Pageable pageable) {
    ...
    @PostMapping
    public ResponseEntity<Void> createEditor(@RequestBody CreateEditorRequest createEditorRequest) {
        editorUseCases.createEditor(createEditorRequest);
        return ResponseEntity.status(HttpStatus.CREATED).build();
    }

    @PatchMapping("/{id}")
    public ResponseEntity<Void> updateEditor(@PathVariable Long id, @RequestBody UpdateEditorRequest updateEditorRequest) {
